...
def backfill_userids(list_id, endpoint):
    page = 1
    numPages = 1
    while page <= numPages:
        resp = requests.get(
...
if Config.CAMPAIGN_MONITOR_REGISTERED_USERS_LIST and Config.CAMPAIGN_MONITOR_API_URL and Config.PIANO_ESP_API_URL:
    for list_id in Config.CAMPAIGN_MONITOR_REGISTERED_USERS_LIST.split(','):
        backfill_userids(list_id, '/active.json')
        backfill_userids(list_id, '/unsubscribed.json')
        backfill_userids(list_id, '/deleted.json')